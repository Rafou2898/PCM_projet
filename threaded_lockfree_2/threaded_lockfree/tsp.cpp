...
#include "lock_free_stack_runner.hpp"
/*****************************************************************
...
int main(int argc, char** argv) {
	...
	TSPTask tsp4;
	tsp4.cutoff(8);
	StackTaskRunner r4(std::thread::hardware_concurrency(), TSPPath::MAX_GRAPH);
	r4.run(&tsp4);
	std::cout << "stack: " << tsp4.result() << " t:" << r4.duration() << std::endl;
	...
	// TSPTask tsp2;
	// tsp2.cutoff(8);
	// PartitionedTaskStackRunner r2(TSPPath::MAX_GRAPH);
	// r2.run(&tsp2);
	// std::cout << "partit: " << tsp2.result() << " t:" << r2.duration()
	// 	  << " s:" << r2.solves() << "/" << r2.splits() << std::endl;
